...
import json
import re
import plotly.express as px # BUG 8: Movido al inicio
...
def verificar_motor():
    if os.path.exists(HEARTBEAT_FILE):
        try:
            ...
            if time.time() - last_heartbeat < 180: return "ONLINE 🟢"
        except: pass
    return "OFFLINE 🔴"
    ...
    try:
        ...
        for e in resp.json():
            ...
            if vol > 10000:
                ...
                mercados.append({"titulo": e.get("title", "N/A"), "volumen": vol, "precio": float(prices[0]) if prices else 0, "volatilidad": abs(e.get("oneDayPriceChange", 0))})
    except Exception: pass
    
    if not mercados: return "Sin datos."
...
def misiones_argo(api_key, saldo_actual, backend="Groq"):
    ...
    investigador = Agent(role="Analista", goal="Elegir mercado.", backstory="Analista.", llm=modelo_ia, verbose=True)
    gestor = Agent(role="Riesgos", goal="Monto inversión.", backstory="Actuario.", llm=modelo_ia, verbose=True)
    critico = Agent(role="Auditor", goal="Veredicto JSON.", backstory="Crítico.", llm=modelo_ia, verbose=True)

    tarea1 = Task(description=f"Analiza:\n{datos_mercado}", expected_output="Nombre mercado.", agent=investigador)
    tarea2 = Task(description="Propón monto (máx 5%).", expected_output="Monto sugerido.", agent=gestor)
    tarea_final = Task(description="JSON FINAL: {accion, mercado, precio_clob, take_profit, stop_loss, monto, razonamiento, evidencias}", expected_output="JSON puro.", agent=critico)

    # BUG 7: Crew construida y kickoff() ejecutado
    crew = Crew(agents=[investigador, gestor, critico], tasks=[tarea1, tarea2, tarea_final], verbose=False)
...
st.markdown("<p style='text-align: center; color: #94a3b8;'>Red Neuronal de Trading Autónomo en Polymarket</p>", unsafe_allow_html=True)

stat_col1, stat_col2, stat_col3 = st.columns(3)
stat_col1.metric("Cartera Simulada", f"${st.session_state['saldo']:.2f}")
stat_col2.metric("Operaciones", len([x for x in st.session_state["historial"] if x.get("Acción") == "COMPRAR"]))
...
with col_side:
    ...
    api_key_input = api_key_env or st.text_input("Ingresar Groq API Key (Manual)", type="password")
    ...
    if st.button("🚀 Ejecutar Análisis Manual", use_container_width=True):
        with st.spinner("Ejecutando agentes..."):
            res_bruto = misiones_argo(api_key_input, st.session_state["saldo"], backend=backend_mode)
            try:
                ...
                if match:
                    ...
                    if status == "COMPRAR": st.session_state["saldo"] -= monto
                    st.success(f"Detección: {status}")
                st.rerun()
            except: st.error("Error al procesar JSON")
...
with col_main:
    ...
    with tabs[0]:
        ...
        else: st.info("Esperando escaneo...")
