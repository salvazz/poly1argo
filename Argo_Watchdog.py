...
PYTHON_PATH = os.path.join(
    ...
    "python3" if os.name != "nt" else "python.exe",
)
STREAMLIT_PATH = os.path.join(
    BASE_DIR,
    "venv",
    "bin" if os.name != "nt" else "Scripts",
    "streamlit" if os.name != "nt" else "streamlit.exe",
)
...
def check_process(name_pattern):
    """Verifica si un proceso existe usando psutil."""
    for proc in psutil.process_iter(["pid", "name", "cmdline"]):
        if proc.info["name"] == "python.exe" and name_pattern in " ".join(
...
def start_agents():
    ...
    p_path = (
        os.path.join(BASE_DIR, PYTHON_PATH)
        if not PYTHON_PATH.startswith("/")
        else PYTHON_PATH
    )
    s_path = (
        os.path.join(BASE_DIR, STREAMLIT_PATH)
        if not STREAMLIT_PATH.startswith("/")
        else STREAMLIT_PATH
    )
    ...
    if os.name != "nt":
        subprocess.Popen(
            f"nohup {PYTHON_PATH} Argo_Motor_24x7.py > motor.log 2>&1 &",
            shell=True,
            cwd=BASE_DIR,
        )
        subprocess.Popen(
            f"nohup {STREAMLIT_PATH} run Argo_Dashboard_Autonomo.py --server.port 8501 > dashboard.log 2>&1 &",
            shell=True,
            cwd=BASE_DIR,
        )
    else:
        ...
        subprocess.Popen(
            f"start /B python Argo_Motor_24x7.py", shell=True, cwd=BASE_DIR
        )
        subprocess.Popen(
            f"start /B streamlit run Argo_Dashboard_Autonomo.py --server.port 8501",
            shell=True,
            cwd=BASE_DIR,
        )
...
def poll_telegram():
    ...
    while True:
        try:
            r = requests.get(
                url, params={"offset": last_update_id + 1, "timeout": 20}, timeout=25
            )
            data = r.json()
            if data.get("ok"):
